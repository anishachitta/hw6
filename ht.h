...
private:
    ...
    double resizeAlpha;
    size_t numItems;     // Number of non-deleted items in the bucket
};
...
{
    bool isEmpty = true;
    for (size_t val = 0; val < table_.size(); val++)
    {
        // item exists
        if( (table_[val]) && (table_[val]->deleted == false))
        {
            isEmpty = false;
        }
    }
...
{
    // Check if resizing is needed based on the loading factor
    if (static_cast<double>(numItems) / table_.size() >= resizeAlpha)
    {
        resize();
    }

    // Probe for a location to insert/update the item
    HASH_INDEX_T loc = probe(p.first);

    // If probing fails to find an empty slot, throw an error
    if (loc == npos)
    {
        throw std::logic_error("Insertion failed. No space found even after resize.");
    }

    // If an empty slot is found, insert the new item
    if (table_[loc] == nullptr)
    {
        table_[loc] = new HashItem(p);
        numItems++;  // Increment number of non-deleted items
    } 
    // If a slot with the same key is found, update the value
    else if (kequal_(table_[loc]->item.first, p.first))
    {
        table_[loc]->item.second = p.second;  // Update value
    }
    // If the slot is marked as deleted, insert the new item
    else
    {
        delete table_[loc];  // Cleanup deleted item
        table_[loc] = new HashItem(p);  // Insert new item
        numItems++;  // Increment number of non-deleted items
    }
}
...
{
    HASH_INDEX_T hash = this->probe(key);

    if (npos == hash)
    {
        return;
    }
    else if (table_[hash] != nullptr)
    {
        table_[hash]->deleted = true;
    }
...
typename HashTable<K, V, Prober, Hash, KEqual>::ItemType const *HashTable<K, V, Prober, Hash, KEqual>::find(const KeyType &key) const
{
    ...
    if ((npos == h) || table_[h] == nullptr || table_[h]->deleted)
    {
        return nullptr;
    }
...
void HashTable<K, V, Prober, Hash, KEqual>::resize() {
    mIndex_++; // Increment the index

    if (mIndex_ >= table_.size()) { // Check if index is beyond table size
        throw std::logic_error("No more capacities available for resizing."); // Throw an exception if no more capacities are available
        return; // Return from the function
    }

    std::vector <HashItem *> prevTable = table_; // Store the previous table
    std::vector <HashItem *> resizedTable(CAPACITIES[mIndex_]); // Create a resized table with the next capacity

    // Update the table to the resized table
    table_ = resizedTable;

    // Iterate through the previous table
    for (int i = 0; i < prevTable.size(); i++) {
        ...
        if (prevTable[i] != nullptr) {
            ...
            if (prevTable[i]->deleted == false) {
                ...
                HASH_INDEX_T newInsert = probe(prevTable[i]->item.first);
                // Insert the item into the new table
                table_[newInsert] = prevTable[i];
            } else {
                // Delete the item and decrement the count of items
                delete prevTable[i];
                numItems--;
            }
        }
